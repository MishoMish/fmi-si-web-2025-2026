...
- Голяма част от потребителите открива уеб сайтовете от мобилни устройства
- Google приоритизира мобилни версии при индексиране
- Един код за всички екрани — по-лесно поддържане
...
> `viewport` помага да се избегне нежелано автоматично мащабиране, което някои мобилни браузъри правят при смяна на ориентацията.
...
### Mobile-first надграждане по ширина

```css
/* База: мобилни (без media query) */
body {
    font-size: 14px;
}
...
В практиката най-често надграждаме само с `min-width`, защото кодът остава по-чист и DRY (Don't Repeat Yourself). `max-width` също съществува, но обикновено се използва по-рядко и в по-специфични случаи.

## Breakpoints в реалната практика

Важно: в индустрията не мислим в твърди категории „телефон/таблет/лаптоп“, защото има огромно разнообразие от размери и плътности.

Най-добрият подход е:

1. Изградете дизайна mobile-first.
2. Намалявайте/увеличавайте ширината в DevTools.
3. Добавете breakpoint там, където layout-ът започва да се „чупи“.

Полезни начални стойности (ориентир, не правило):

| Цел | Примерна стойност |
|-----|--------------------|
| Първо надграждане | `@media (min-width: 768px)` |
| Второ надграждане | `@media (min-width: 1024px)` |
| По-големи екрани при нужда | `@media (min-width: 1280px)` |
...
```css
/* Базово безопасно правило за responsive изображения */
img {
    max-width: 100%;
    height: auto;
}

/* Пример за допълнително правило при висока плътност */
...
5. **Поддържайте читаемост** — добър контраст, достатъчен размер на шрифта (минимум 16px)
